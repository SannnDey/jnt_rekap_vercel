...
export default function LoginPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      router.push(user.role === 'developer' ? '/manage-users' : '/');
    } catch (error) {
      toast(error instanceof Error ? error.message : 'Login gagal', 'error');
    }
  };
    ...
    <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 py-10">
      <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-slate-900/90 p-8 shadow-2xl shadow-slate-950/30">
        <div className="space-y-2">
          ...
          <h1 className="text-3xl font-semibold text-white">Masuk ke sistem</h1>
          <p className="text-sm text-slate-400">Gunakan akun yang sudah disetujui admin untuk mengakses aplikasi.</p>
        </div>
